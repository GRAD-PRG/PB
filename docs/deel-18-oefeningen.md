...
De code in de `Sort` method van `List<T>` gebruikt een bepaald [sorteer algoritme](https://nl.wikipedia.org/wiki/Sorteeralgoritme) om de elementen op de juiste plaats te krijgen.

Er zijn vele [soorten sorteer algoritmen](https://www.youtube.com/watch?v=ZZuD6iUe3Pc), die o.a. verschillen in
...
Dit soort informatie zou in [de documentatie](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1.sort) moeten staan. In de 'Remarks' sectie vinden we inderdaad :
...
2.  gesorteerd op naam a→z

3.  gesorteerd op naam z→a
...
**Schrijf een *immutable* klasse `Interval`** met volgende members:
...
**Schrijf een klasse `MinDanLengteComparer`**, die is een *comparer* die we bv. kunnen gebruiken om met `Sort()` een verzameling `Interval` objecten te sorteren.
...
Veruit het moeilijkste stuk hiervan zal de `Overlapt` method zijn, die beslist of deze afspraak (*this*) en de andere afspraak (via de parameter) met elkaar overlappen.
...
![Enkele overlappende afspraken](images/d18-afspraak-overlap.jpg)

Je ziet bv. dat A1 en A3 NIET overlappen maar dat A4 en A3 WEL overlappen.

**Schrijf ook een klasse `TijdsduurComparer`** die we kunnen gebruikt om een lijst van afspraken te sorteren, op basis van hun tijdsduur (d.w.z. hoe lang ze duren). De sorteervolgorde moet "*van korter naar langer*" zijn.
...
> **Tip**
>
> Probeer ze niet alle drie tegelijk te schrijven. Begin met `AllCapStyle` (de gemakkelijkste) en probeer het programma uit. Zet de regels voor de andere stijlen voorlopig in commentaar.
