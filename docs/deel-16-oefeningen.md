...
Zoek in de documentatie de onderstaande [List&lt;T&gt;](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.list-1) methods op.
...
Zoek in de documentatie de onderstaande [LinkedList&lt;T&gt;](https://docs.microsoft.com/en-us/dotnet/api/system.collections.generic.linkedlist-1) methods op.
...
Je zult in de documentatie het type `IEnumerable<T>` tegenkomen. Dit is een algemeen type dat eender welke collectieklasse kan voorstellen : `List<T>`, `LinkedList<T>` of `HashSet<T>`. Wat het verband is tussen `IEnumerable<T>` en deze klasse, zien we in een later deel over *interfaces*.
...
In deze oefening gaan we een simpele *social media* filter bouwen die 'stoute woordjes' kan censureren.
...
1.  een lijst met woorden

2.  een set met verboden woorden
...
>
> -   Je kunt een string splitsen in een array van woorden met de `Split` method uit [Deel-09](../deel-09/deel-09.html).
>
...
1.  een lijst met `Student` objecten

2.  een gemeente (een `string`)
...
>
> In de uitleg van [Deel-16](../deel-16/deel-16.html) worden twee manieren besproken hoe je dit correct kunt doen.
