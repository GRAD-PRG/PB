...
Let erop dat hierboven geen *magic value* (of `const int`) `52` voorkomt, we hebben het totaal aantal kaarten gewoon berekend.
...
Voor de output werd ook eens iets anders gedaan dan een loop, *gewoon omdat het kan* ;)
...
Bij de *Durstenfeld shuffle* beginnen we op de laatste positie en gaan per iteratie eentje dichter naar de eerste positie. Die eerste positie (index `0` dus) zelf gebruiken we echter niet, we stoppen eentje eerder.
...
Je ziet dat deze oplossing heel wat eenvoudiger is dan [de oplossing van D08toongetallen](../deel-08-oplossingen/deel-08-oplossingen.html#_oplossing_d08toongetallen)!
...
### Oplossing D09censuuruitbreiding ☒
Voor deze oefening is er geen voorbeeld oplossing beschikbaar.
