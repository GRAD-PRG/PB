...
In het geheugen als volgt (schematisch) kan voorgesteld worden : ![lottocijfers](images/lottocijfers.png)
...
Bij de method oproep wordt dus de waarde van variabele `lottocijfers` gekopieerd naar de opslagplaats voor de parameter `getallen` en dan ziet de situatie in het geheugen er als volgt uit : ![lottocijfers en getallen](images/lottocijfers-en-getallen.png)
...
De method `VerlaagGetallen` zal dus waarden in dit array veranderen : ![lottocijfers en getallen verlaagd](images/lottocijfers-en-getallen-verlaagd.png)

Eenmaal de method `VerlaagGetallen` is afgelopen, keren we terug naar de `Main` method met deze situatie : ![lottocijfers en getallen verlaagd](images/lottocijfers-verlaagd.png)
...
Op het moment dat de uitvoering bij regel &lt;2&gt; aankomt, ziet het er in het geheugen zo uit : ![willekeurige getallen](images/willekeurige-getallen.png)
...
Als de uitvoering aankomt bij de for loop in `Main` ziet het er in het geheugen dan zo uit : ![willekeurige getallen](images/getallen.png)
...
4.  dit datatype heeft de naam `Tarief` en heeft 2 mogelijke waarden
...
Maar wat als we slechts 1 enkele toetsdruk nodig hebben? Bijvoorbeeld voor de klassieke [To start press any key](https://www.youtube.com/watch?v=st6-DgWeuos).

Er bestaat een `Console.ReadKey()` method die precies doet wat we nodig hebben :

1.  wacht tot de gebruiker op een toets drukt

2.  de return value vertelt ons om welke toets het ging

In de praktijk ligt het helaas wat moeilijker.

-   Het console venster houdt een buffer bij van ingetypte symbolen die nog niet door een `ReadLine` of `ReadKey` werden verwerkt. Als de gebruiker de A toets ingedrukt houdt en het programma doet pas over 2 seconden een `ReadLine`, dan zul je zien dat er onmiddellijk al een ganse resem `A`'s verschijnen.
