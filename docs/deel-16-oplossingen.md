...
T item
-
voegt `item` achteraan de lijst toe
...
int index
T item
voegt `item` toe op positie `index`
...
T item
-
zoekt vanaf begin van de lijst naar `item`  
...
T item
int index
idem maar begint de zoektocht op positie `index`
...
T item
-
verwijdert `item` uit de lijst (enkel eerste voorkomen!)  
...
int index
-
verwijdert het element op positie `index`
...
Sorteert de elementen volgens hun **"natuurlijke" volgorde** :
...
AddFirst
T value
voegt `value` vooraan de lijst toe  
...
AddLast
T value
voegt `value` achteraan de lijst toe  
(de return value is voor ons niet interessant)
...
Add
T item
voegt `item` toe aan de set. De return value geeft aan of er daadwerkelijk iets werd toegevoegd (false indien reeds aanwezig)
...
ExceptWith
IEnumerable&lt;T&gt; other
verwijdert alle elementen van collectie `other` uit deze set,  
vergelijkbaar met [het verschil uit de verzamelingenleer](https://nl.wikipedia.org/wiki/Verschil_(verzamelingenleer))


void
IntersectWith
IEnumerable&lt;T&gt; other
verwijdert alle elementen uit deze set die *niet* in `other` voorkomen  
vergelijkbaar met [de doorsnede uit de verzamelingenleer](https://nl.wikipedia.org/wiki/Doorsnede_(verzamelingenleer))


void
UnionWith
IEnumerable&lt;T&gt; other
voegt alle elementen van `other` toe aan deze set (zonder dubbels natuurlijk)  
vergelijkbaar met [de unie of vereniging uit de verzamelingenleer](https://nl.wikipedia.org/wiki/Vereniging_(verzamelingenleer))


bool
Overlaps
IEnumerable&lt;T&gt; other
Bepaalt of deze set en `other` minstens één element gemeenschappelijk hebben. De return value `true` betekent dat er zo’n element gevonden werd (d.w.z. hun doorsnede is niet leeg).
...
1.  stop de unieke getallen in een lijst, zodat we die kunnen sorteren

2.  sorteer de lijst met de `Sort()` method

    -   de getallen staan nu in oplopende volgorde, van klein naar groot

3.  keer de volgorde van de elementen in de lijst om met de `Reverse()` method
...
1.  bouw een array met woorden, door de string te splitsen met de .Split() method

2.  bepaal de unieke woorden, door een HashSet te vullen met de inhoud van het array met woorden

3.  stop de unieke woorden uit de HashSet in een lijst en sorteer deze (alfabetisch)

4.  toon het aantal unieke woorden (i.e. het aantal elementen in de HashSet)

5.  toon de gesorteerde woorden
...
1.  Merk op dat deze `PrintStudent` method `static` is, het is immers geen *instance method* die we op `Program` objecten willen toepassen.
